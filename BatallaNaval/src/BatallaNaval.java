...
public class BatallaNaval{
    ...
    public static void main (String[] args){
        try{ 
            int anchoH = 10;        // Ancho horitzontal tablero
            int anchoV = 10;        // Ancho vertical tablero
            int numBarcos= 4;       // Número de barcos a colocar
            int medidaMinima = 2;   // Medida mínima de los barcos
            int numIntentos = 50;    // Número de intentos
            ...
            agua(tablero); //Llamamos a agua para rellenar el tablero -ancho y alto declarado arriba-
            ...
            System.out.println("");
            do{ 
                extraidos = jugar (tablero, intento, x);
                intento = extraidos[1];
                numX = numX+extraidos[0];
            }while (intento <=numIntentos && numX < x );
            ...
            if(intento>=numIntentos){
                ...
                System.out.println("¡Estás hecho todo un Blas de Lezo! Has ganado la batalla naval.");
            }
        }   
    ...
    static void colocaBarco(char[][] tablero, int mida){
        ...
        do{
            ...
            for ( int e = -1, i = 0; e<=mida+1 && agua == true; e++){ //ampla
                for (int o = -1; o<2 && agua == true; o++){
                    if(ranorient == 0){ //avanza misma fila
                        if((ranFila+o<=tablero.length-1 && ranFila+o>=0) && (ranCol+e<=tablero[tablero.length-1].length-1 && ranCol+e>=0)){
                    ...
                    else if(ranorient == 1){ //avanza misma columna
                        if((ranCol+o<=tablero[tablero.length-1].length-1 && ranCol+o>=0)&& (ranFila+e<=tablero.length-1 && ranFila+e>=0)){                              
    ...
    static int[] jugar (char[][] tablero, int intento, int x){
        ...
        try{
            ...
            if( horizontal>=1 && horizontal<=10 && vertical>=1 && vertical<=10){
                ...
                else if(tablero[vertical][horizontal]=='x'){
                    System.out.println("¡Ya habías disparado en esta casilla! Debes estar más atento");
                }               
 
            }
 
            else {
                System.out.println("¡El tablero sólo tiene 10 posiciones! -Escoge del 1 al 10-");
            }
            System.out.println ("");            
            imprimir(tablero, mostrar);
            extraer[0]= x;
